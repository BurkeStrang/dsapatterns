...
// generateGeneralizedAbbreviation returns all abbreviations for the given word
...
// generateAbbreviationRecursive is a recursive helper function to generate abbreviations
func generateAbbreviationRecursive(word string, abWord string, start int, count int, result *[]string) {
	if start == len(word) {
		if count != 0 {
		...
		*result = append(*result, abWord)
		...
		generateAbbreviationRecursive(word, abWord+string(word[start]), start+1, 0, result)
	}
}
